    ...
    "work for all values (except checks)" in {
      SbtReactiveApp.labels(
        ...
        endpoints = Vector(
          ...
          HttpEndpoint("ep2", 5678, HttpIngress(ingressPorts = Vector(80, 443), hosts = Vector("hello.world.com", "goodbye.cruel.world.com"), paths = Vector("^/my-url.*$", "^/my-other-url.*$"))),
          TcpEndpoint("ep3", 9123, PortIngress(8080)),
          UdpEndpoint("ep4", 8123, PortIngress(8080)),
          HttpEndpoint("ep5", 1235, ingress = None, version = None),
          HttpEndpoint("ep6", 1236, ingress = None, version = Some(LiteralVersion("buggy123"))),
          HttpEndpoint("ep7", 1237, ingress = None, version = Some(MajorMinorVersion))
        ),
        volumes = Map(
        ...
        "com.lightbend.rp.endpoints.1.ingress.type" -> "http",
        "com.lightbend.rp.endpoints.1.ingress.paths.0" -> "^/my-url.*$",
        "com.lightbend.rp.endpoints.1.ingress.paths.1" -> "^/my-other-url.*$",
        "com.lightbend.rp.endpoints.1.ingress.hosts.0" -> "hello.world.com",
        "com.lightbend.rp.endpoints.1.ingress.hosts.1" -> "goodbye.cruel.world.com",
        "com.lightbend.rp.endpoints.1.ingress.ingress-ports.0" -> "80",
        "com.lightbend.rp.endpoints.1.ingress.ingress-ports.1" -> "443",
