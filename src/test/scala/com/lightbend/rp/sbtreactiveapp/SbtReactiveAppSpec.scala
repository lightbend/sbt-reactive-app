  ...
  "labels" should {
    ...
    "work for all values (except checks)" in {
      SbtReactiveApp.labels(
        ...
        nrOfCpus = Some(0.25),
        endpoints = Map(
          "ep1" -> HttpEndpoint(1234),
          "ep2" -> HttpEndpoint(5678, HttpPathIngress("^/my-url.*$"), HttpHostIngress("hello.world.com")),
          "ep3" -> TcpEndpoint(9123, PortIngress(8080)),
          "ep4" -> UdpEndpoint(8123, PortIngress(8080))
        ),
        volumes = Map(
